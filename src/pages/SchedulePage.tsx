...
import { BookPlus, CalendarPlus, Clock, MapPin, User, CalendarRange } from 'lucide-react'
          ...
          {dayEntries.length > 0 ? (
            <div className="animate-fade-in space-y-3" key={activeDay}>
              {dayEntries.map((entry) => {
                ...
                const tone = subject ? toneFor(subject.tone) : null
                return subject ? (
                  <Pressable
                    key={entry.id}
...
function WeeklyGrid({
  ...
  return (
    <div className="mt-2 min-w-0 overflow-x-auto pb-2">
      <div className="grid min-w-[680px] grid-cols-[60px_repeat(7,1fr)] gap-px overflow-hidden rounded-3xl border border-zinc-200/80 bg-zinc-100/70 dark:border-zinc-800 dark:bg-zinc-800">
